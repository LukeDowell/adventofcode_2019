...
fun main() {
    val input = InputReader("Day03Input.txt").get()
    println("Part one: ${Day03(input).partOne}")
    println("Part two: ${Day03(input).partTwo}")
}


class Day03(input: String) {
    data class Point(val x: Int, val y: Int) {
        fun allPointsTo(dir: String): List<Point> {
            val numSteps = dir.substring(1).toInt()
            return when (dir[0]) {
                'R' -> { (1..numSteps).map { Point(this.x + it, this.y) } }
                'L' -> { (1..numSteps).map { Point(this.x - it, this.y) } }
                'D' -> { (1..numSteps).map { Point(this.x, this.y - it) } }
                else -> { (1..numSteps).map { Point(this.x, this.y + it) } }
            }
        }
    }
    ...
    init {
        val wireOnePath = input.split("\n")[0].split(",")
        val wireTwoPath = input.split("\n")[1].split(",")

        fun calcAllPointsForWirePath(wirePath: List<String>): List<Point> {
            var pointPath: List<Point> = listOf(Point(0, 0))

            wirePath.forEach { v ->
                pointPath = pointPath + pointPath.last().allPointsTo(v)
            }

            return pointPath
        }

        val wireOne = calcAllPointsForWirePath(wireOnePath)
        val wireTwo = calcAllPointsForWirePath(wireTwoPath)

        val sharedPoints = wireOne.filter { wireTwo.contains(it) }.filter { it != Point(0, 0) }

        this.partOne = sharedPoints.map { kotlin.math.abs(it.x) + kotlin.math.abs(it.y) }.min() ?: Int.MAX_VALUE
        this.partTwo = sharedPoints.map { wireOne.indexOf(it) + wireTwo.indexOf(it) }.min() ?: Int.MAX_VALUE
    }
}